...
Then this evaluator would prefer prospect $y(a)$ to prospect $y(b)$, since the expected value of these log measures is $1.29$ for Prospect $y(a)$ and $1.22$ for Prospect $y(b)$, as seen in the row labeled $\mathrm{E}\big[\mathrm{E}[\mathrm{log} \, y]\big]$ in the table above.
...
We then have two representations of the evaluator's social preference over known outcome distributions. They are equivalent in the sense that they both correctly represent the social preference. However, once we add uncertainty and ambiguity about what outcome distribution arises with a given treatment, their recommendations as to which is the preferred treatment differ. The question is: which, if any, is giving the right answer? 
...
## Egalitarian Equivalent
 ...
 The *egalitarian equivalent* $ee = \mathcal{EE}(y_1,...,y_n)$ of a distribution of outcomes $(y_1,...,y_n)$ is the outcome level $ee = \mathcal{EE}(y_1,...,y_n)$ such that, the evaluator would be indifferent between the distribution $(y_1,...,y_n)$ and $(\underbrace{ee,..., ee}_{n-times})$. The egalitarian equivalent is therefore computed as follows:
 
$$ \mathcal{EE}(y_1,...,y_n):=f^{-1}\left(\frac{f(y_1)+...+f(y_n)}{n}\right). $$ 

For an inequality averse evaluator,
...
Theorem 3.1 in [Social Preferences Under Uncertainty and Ambiguity](https://osf.io/25nme/), following [Fleurbaey 2010](https://www.jstor.org/stable/10.1086/656513), shows that the following is true:[^2] 
...
[^2]: Under standard continuity, dominance, and Pareto conditions.
