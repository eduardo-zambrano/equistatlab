...
Our evaluator is inequality averse and evaluates outcome distributions for each state $s$ using the magnitude ${\frac{1}{3}}(f(y_1^s) + f(y_2^s) + f(y_3^s))$, where $f$ is a strictly increasing and strictly concave function. In this example, we let $f(y)=\ln y$. 
